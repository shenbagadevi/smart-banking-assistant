...
def _infer_product_metadata(
    ...
    """
    Extract business metadata from document content.
    """
    ...
    if "home loan" in text:
        ...
        metadata["product_category"] = "loan"
        metadata["loan_type"] = "personal_loan"
        metadata["product_name"] = "Personal Loan"

    elif "car loan" in text:
        metadata["product_category"] = "loan"
        metadata["loan_type"] = "vehicle_loan"
        metadata["product_name"] = "Vehicle Loan"

    elif "credit card" in text:
        metadata["product_category"] = "card"
        metadata["product_name"] = "Credit Card"
...
def parse_document(file_path: Path) -> list[dict]:
    ...
    try:
        ...
        product_category = _infer_product_metadata(
            file_path.name,
            parsed_elements,
        )
...
def _get_page_number(item):
    """Extract page number from Docling item provenance with logging."""
    try:
        page = getattr(item, "page_no", None)
        if page is not None:
            logger.debug("PAGE FOUND | page=%s (direct)", page)
            return page
        ...
        if prov:
            ...
            if page is not None:
                logger.debug("PAGE FOUND | page=%s (prov)", page)
                return page
